...
app.get("/searchSubtitles", async (req, res) => {
  try {
    ...
    if (url) {
      ...
      subUrl = `https://api.opensubtitles.com/api/v1/subtitles?moviehash=${hash}&languages=${lang}`;
    } else if (title) {
      subUrl = `https://api.opensubtitles.com/api/v1/subtitles?query=${title}&languages=${lang}`;
    }
